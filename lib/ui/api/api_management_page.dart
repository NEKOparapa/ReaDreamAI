...
String get _configKey => widget.apiType == ApiType.language ? 'languageApis' : 'drawingApis';
String get _activeIdKey => widget.apiType == ApiType.language ? 'activeLanguageApiId' : 'activeDrawingApiId';
String get _newApiName => widget.apiType == ApiType.language ? '新语言接口' : '新绘画接口';
...
void _addApi() async {
if (widget.apiType == ApiType.language) {
final newApi = ApiModel.create(_newApiName);
final result = await Navigator.push<ApiModel>(
context,
MaterialPageRoute(builder: (context) => ApiSettingsPage(apiModel: newApi)),
);
if (result != null) _onApiUpsert(result, isNew: true);
} else { 
final newApi = ApiModel.createDrawing(_newApiName); 
final result = await Navigator.push<ApiModel>(
context,
MaterialPageRoute(builder: (context) => DrawingApiSettingsPage(apiModel: newApi)),
);
if (result != null) _onApiUpsert(result, isNew: true);
}
}
...
final page = widget.apiType == ApiType.language
? ApiSettingsPage(apiModel: apiToEdit)
: DrawingApiSettingsPage(apiModel: apiToEdit);
...
void _testApi(ApiModel api) {
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      ...
      duration: const Duration(minutes: 1), // 设置一个较长的持续时间
    ),
  );

  // 使用 Future.microtask 确保测试代码在下一帧运行，避免阻塞UI
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          duration: const Duration(seconds: 6), // 结果显示6秒
        ),
      );
    }
  });
